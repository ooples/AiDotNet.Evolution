// Migrated from ooples/AiDotNet 66d7602c92101e5ab2bd9db8cfa7f7526fa2c75d:src/AiDotNet.Evolution.CSharp/CSharpProposalSource.cs
...
internal sealed class CSharpProposalSource : ICostedProgramProposalSource
{
    ...
    private readonly ICSharpProposalClient _client;
    ...
    private CSharpProposalSource(ICSharpProposalClient client, CSharpProgramEvolutionOptions options, CSharpProgramSourceOptions program,
    ...
    internal static CSharpProposalSource Create(ICSharpProposalClient client, CSharpProgramEvolutionOptions options,
        CSharpProgramSourceOptions program, EvolutionResourceLedger ledger)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            WriteEvidence(context, 0, string.Empty, string.Empty, null, "The parent has no valid bounded editable syntax.", work, Array.Empty<CompilerChatMessage>());
        ...
        CompilerChatMessage system = CompilerChatMessage.System(Instructions), initial = CompilerChatMessage.User(JsonSerializer.Serialize(data));
        var messages = new List<CompilerChatMessage> { system, initial };
        for (int attempt = 0; attempt <= _options.MaxRepairs; attempt++)
        {
            ...
            CompilerChatResponse response;
            try
            {
                response = await _client.GetResponseAsync(messages, new CompilerChatOptions
                {
                    Temperature = 0.2,
            ...
            messages = new List<CompilerChatMessage> { system, initial, CompilerChatMessage.Assistant(text), CompilerChatMessage.User(result.Feedback) };
        }
        Interlocked.Increment(ref _abandoned);
    ...
    private void WriteEvidence(EvolutionVariationContext<ProgramGenome> context, int attempt, string response, string model,
        CSharpPatchAttempt? result, string feedback, Work work, IReadOnlyList<CompilerChatMessage> messages)
    {
        work.Audits++;
        ProgramGenome parent = context.Parent.Candidate.CanonicalGenome.Genome;

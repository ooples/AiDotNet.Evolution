...
public static class EvolutionTraceFile
{
    ...
    internal static string SerializeRecord(EvolutionTraceRecord record) =>
        ToJson(record).ToJsonString(EvolutionJson.Compact);
    ...
    internal static string JsonFooter(EvolutionTraceSummary? summary)
    {
        ...
        if (summary is not null)
        {
            builder.Append(",\n  \"").Append(SummaryPropertyName).Append("\": ")
                .Append(SummaryToJson(summary).ToJsonString(EvolutionJson.Compact));
        }
        builder.Append("\n}\n");
        return builder.ToString();
    }
    ...
    internal static JsonObject ToJson(EvolutionTraceRecord record)
    {
    ...
    internal static EvolutionTraceRecord FromJson(JsonObject json)
    {
        Guard.NotNull(json);
        try
        {
    ...
    internal static JsonObject SummaryToJson(EvolutionTraceSummary summary)
    {
    ...
    /// <exception cref="InvalidDataException">A required field is missing or malformed.</exception>
    internal static EvolutionTraceSummary SummaryFromJson(JsonObject json)
    {
        Guard.NotNull(json);
        var statusCounts = new Dictionary<EvolutionEvaluationStatus, long>();
        if (json["statusCounts"] is JsonObject counts)
        {
            foreach (KeyValuePair<string, JsonNode?> pair in counts)
            {
                ...
                statusCounts[status] = pair.Value.GetValue<long>();
            }
        }
    ...
    private static EvolutionTraceRecord? TryParseLine(string line)
    {
        try
        {
            JsonObject? json = JsonNode.Parse(line) as JsonObject;
    ...
    private static bool IsRecordLine(string line)
    {
        ...
        try
        {
            JsonObject? json = JsonNode.Parse(trimmed) as JsonObject;
    ...
    {
        byte[] utf8 = TraceEncoding.GetBytes(content);
        ...
        try
        {
            while (reader.Read())
            {
                if (reader.TokenType != JsonTokenType.PropertyName ||
                    !reader.ValueTextEquals(RecordsPropertyName))
                {
                    continue;
                }
    ...
    private static JsonObject ValuesToJson(IReadOnlyDictionary<string, double> values)
    {
        var json = new JsonObject();
    ...
    private static Dictionary<string, double> ValuesFromJson(JsonObject json, string propertyName)
    {
        var values = new Dictionary<string, double>(StringComparer.Ordinal);
        if (json[propertyName] is not JsonObject source) return values;
        foreach (KeyValuePair<string, JsonNode?> pair in source)
            if (pair.Value is not null) values[pair.Key] = pair.Value.GetValue<double>();
        return values;
    }

    private static List<string> IdentitiesFromJson(JsonObject json, string propertyName)
    {
        var values = new List<string>();
        if (json[propertyName] is not JsonArray source) return values;
        foreach (JsonNode? token in source)
        {
    ...
    private static List<EvolutionDiagnostic> DiagnosticsFromJson(JsonObject json)
    {
        var diagnostics = new List<EvolutionDiagnostic>();
        if (json["diagnostics"] is not JsonArray source) return diagnostics;
        foreach (JsonNode? token in source)
        {
            if (token is not JsonObject element) continue;
            var data = new Dictionary<string, string>(StringComparer.Ordinal);
            if (element["data"] is JsonObject dataObject)
            {
                foreach (KeyValuePair<string, JsonNode?> pair in dataObject)
                {
            ...
            diagnostics.Add(new EvolutionDiagnostic(RequireString(element, "code"),
                element["message"]?.GetValue<string>() ?? string.Empty, Flag(element, "isRedacted"), data));
        }
        return diagnostics;
    }

    private static JsonObject DiagnosticToJson(EvolutionDiagnostic diagnostic)
    {
        var json = new JsonObject
        {
    ...
    private static bool Flag(JsonObject json, string propertyName)
    {
        JsonNode? token = json[propertyName];
        return token is not null && token.GetValue<bool>();
    }

    private static string RequireString(JsonObject json, string propertyName)
    {
    ...
    private static long RequireLong(JsonObject json, string propertyName)
    {
        JsonNode? token = json[propertyName];
    ...
    private static TEnum RequireEnum<TEnum>(JsonObject json, string propertyName) where TEnum : struct
    {
    ...
    private static TEnum? OptionalEnum<TEnum>(JsonObject json, string propertyName) where TEnum : struct
    {
        string? value = json[propertyName]?.GetValue<string>();
    ...
    private static string? OptionalString(JsonObject json, string propertyName)
    {
        string? value = json[propertyName]?.GetValue<string>();
        return value is null || string.IsNullOrWhiteSpace(value) ? null : value;
    }

    private static double? OptionalDouble(JsonObject json, string propertyName)
    {
        JsonNode? token = json[propertyName];
        return token is null ? null : token.GetValue<double>();
    }

    private static long OptionalLong(JsonObject json, string propertyName)
    {
        JsonNode? token = json[propertyName];
        return token is null ? 0L : token.GetValue<long>();
    }

    private static int? OptionalInt(JsonObject json, string propertyName)
    {
        JsonNode? token = json[propertyName];
        return token is null ? null : token.GetValue<int>();
    }

    private static ulong OptionalSeedStream(JsonObject json)
    {
        JsonNode? token = json["seedStream"];
        if (token is null) return 0UL;
        string? text = token.GetValue<string>();

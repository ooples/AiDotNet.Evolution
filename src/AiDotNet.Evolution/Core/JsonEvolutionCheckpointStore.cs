...
/// <remarks>
/// <para>
/// Each save serializes the checkpoint with System.Text.Json into a temporary file in the target directory,
    ...
    private EvolutionCheckpoint? TryLoad(string path, string runId)
    {
        ...
        try
        {
            ...
            if (Encoding.UTF8.GetByteCount(json) > _maxCheckpointBytes)
                throw new InvalidDataException($"The evolution checkpoint exceeds the {_maxCheckpointBytes}-byte limit.");
            CheckpointDocument? document = JsonSerializer.Deserialize<CheckpointDocument>(json, EvolutionJson.Compact);

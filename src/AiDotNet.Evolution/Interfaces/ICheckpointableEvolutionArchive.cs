...
/// their internal indexes from the supplied entries, adopt the supplied descriptor definitions before replay, and adopt
/// the supplied version verbatim rather than recounting insertions, because both descriptor bounds and the version
/// participate in resumed behavior. <see cref="MapElitesArchive{TGenome}"/> is the built-in implementation.
/// </para>
...
{
    /// <summary>Restores descriptor state, entries, and the exact archive version into an empty archive.</summary>
    /// <param name="entries">The elites captured at checkpoint time.</param>
    /// <param name="descriptors">
    /// The descriptor definitions captured at checkpoint time. Implementations must validate and adopt them before
    /// replaying <paramref name="entries"/> so restored cells retain their original meaning.
    /// </param>
    /// <param name="version">

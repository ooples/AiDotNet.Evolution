// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:src/Evolution/Prompts/ProgramPromptBuilder.cs
...
public sealed class ProgramPromptBuilder
{
    ...
    public IReadOnlyList<ProgramChatMessage> BuildEvaluationMessages(
        ProgramGenome program,
        IReadOnlyList<string> criteria,
        string? responseSchema = null)
    {
        ProgramGuard.NotNull(program);
        ProgramGuard.NotNull(criteria);
        if (criteria.Count == 0)
        {
        ...
        return new List<ProgramChatMessage>
        {
            ProgramChatMessage.System(system),
            ProgramChatMessage.User(RenderWithDefaults(_templates.GetTemplate(ProgramPromptTemplateKey.Evaluation), values))
        };
    }
    ...
    {
        ProgramGuard.NotNull(criterion);

// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:src/Evolution/Prompts/ProgramPromptResult.cs
...
public sealed class ProgramPromptResult
{
    ...
    public ProgramPromptResult(
        ...
        bool wasTruncated)
    {
        ProgramGuard.NotNull(systemText);
        ProgramGuard.NotNull(userText);
        ProgramGuard.NotNull(variationChoices);
        ...
        var messages = new List<ProgramChatMessage>(2);
        if (systemText.Length > 0) messages.Add(ProgramChatMessage.System(systemText));
        messages.Add(ProgramChatMessage.User(userText));
        Messages = messages;
    }

    /// <summary>Gets the messages to send, a system message (when non-empty) followed by the user message.</summary>
    public IReadOnlyList<ProgramChatMessage> Messages { get; }

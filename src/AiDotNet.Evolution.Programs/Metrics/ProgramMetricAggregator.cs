// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:src/Evolution/Programs/Metrics/ProgramMetricAggregator.cs
...
/// <summary>Combines typed program metrics under an explicit, validated scoring policy.</summary>
public sealed class ProgramMetricAggregator
{
    ...
    public ProgramMetricAggregationResult Aggregate(IReadOnlyDictionary<string, ProgramMetricValue> metrics)
    {
        ...
        foreach (KeyValuePair<string, ProgramMetricValue> pair in metrics)
        {
            ...
            if (!ordered.TryAdd(pair.Key.Trim(), pair.Value))
                throw new ArgumentException("Metric names collide after trimming.", nameof(metrics));
        }
    ...
    /// <summary>Applies the legacy combined-score-or-mean compatibility policy.</summary>
    ...
    /// <returns>The legacy-policy scalar quality and discarded-value diagnostics; not a current competitor comparison.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="metrics"/> is <c>null</c>.</exception>

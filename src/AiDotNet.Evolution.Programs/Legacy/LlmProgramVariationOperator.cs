// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:src/Evolution/Programs/LlmProgramVariationOperator.cs
...
public sealed class LlmProgramVariationOperator : ICheckpointableVariationOperator<ProgramGenome>, IProgramVariationOperator
{
    ...
    private readonly ProgramProposalOptions _programOptions;
    ...
    /// <see cref="ProgramProposalOptions.Prompt"/>, except that an explicit
    ...
    public LlmProgramVariationOperator(
        ...
        ProgramProposalOptions? programOptions = null,
        ...
        ProposalProvenanceOptions? provenanceOptions = null)
    {
        ...
        ProgramProposalOptions programCopy = (programOptions ?? new ProgramProposalOptions()).Clone();
        ...
        VersionPinnedProgramFitnessEvaluator.ValidateIdentity(_modelId, nameof(chatClient.ModelId));
        ...
        Id = id.Trim();
        VersionHash = EvolutionHash.Combine(new[] { BuildVersionHash(programCopy, variationCopy, _promptBuilder), _modelId });
    }
    ...
    public ProgramProposalOptions GetProgramOptions() => _programOptions.Clone();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        CheckModelIdentity();
        if (_programOptions.Language != ProgramLanguage.Generic && parent.Language != _programOptions.Language)
            throw new ArgumentException("Parent language differs from the configured proposal language.", nameof(context));
        ...
        for (int round = 0; round < attempts * samples; round++)
        {
            ...
            ProgramChatOptions chatOptions = BuildChatOptions(context.Random, round % samples);
            CheckModelIdentity();
            if (messages.Sum(message => (long)message.Text.Length) > _promptBuilder.GetPromptOptions().MaxPromptChars)
                throw new InvalidOperationException("Retry conversation exceeds the configured prompt bound.");
            IReadOnlyList<ProgramChatMessage> request = Array.AsReadOnly(messages.ToArray());
            ...
            try
            {
                ...
                ProgramChatResponse response = await _chatClient
                    .GetResponseAsync(request, chatOptions, cancellationToken)
...
#pragma warning restore CA1031
    ...
    {
        child = parent;
        if (responseText.Length > _programOptions.Diff.MaxResponseChars)
        {
            feedback = "The response exceeds the configured parsing bound.";
    ...
    {
        ProgramGuard.NotNull(state);
        if (state.Length > 1024 * 1024) throw new InvalidDataException("Variation state exceeds its size bound.");

        AttemptStateDocument? document;
        try
        {
            document = JsonConvert.DeserializeObject<AttemptStateDocument>(state, new JsonSerializerSettings { MaxDepth = 16 });
        }
...
#pragma warning restore CA1031
    ...
    private static string RenderConversation(IReadOnlyList<ProgramChatMessage> messages)
    {
        var builder = new StringBuilder();
        foreach (ProgramChatMessage message in messages)
        {
    ...
    private static string HashConversation(IReadOnlyList<ProgramChatMessage> messages)
    {
    ...
    private static ProgramEvolutionPromptOptions DerivePromptOptions(
        ProgramProposalOptions programOptions,
    ...
    private static void AppendFeedback(List<ProgramChatMessage> messages, string? responseText, string feedback)
    {
        messages.Add(ProgramChatMessage.Assistant(
            responseText is null ? "(no response)" : ProgramText.Bound(responseText, MaxFeedbackChars)));
        messages.Add(ProgramChatMessage.User(feedback + " Answer again in the required format."));
    }
        ...
        ProgramPromptBuilder promptBuilder)
    {
        ...
        {
            "llm-program-variation-v6-standalone-bounded-provider",
            ...
            programOptions.Diff.MaxResponseChars.ToString(CultureInfo.InvariantCulture),
    ...
    private void CheckModelIdentity()
    {
        if (!string.Equals(_modelId, _chatClient.ModelId, StringComparison.Ordinal))
            throw new InvalidOperationException("The proposal provider changed model identity during the run.");
    }

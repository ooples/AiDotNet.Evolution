// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:src/Evolution/Programs/ProgramEvolutionTask.cs
...
/// <see cref="ProgramTaskOptions.MaxProgramChars"/>, or one whose evolve-block markers are damaged while
/// <see cref="ProgramTaskOptions.EnforceEvolveBlocks"/> is set, is returned as
...
public sealed class ProgramEvolutionTask : IEvolutionTask<ProgramGenome>, IEvolutionLatencyProfile
{
    ...
    {
        ProgramGuard.NotNull(evaluator);
        ProgramGuard.NotNullOrWhiteSpace(id);

        ProgramTaskOptions effective = (options ?? new ProgramTaskOptions()).Clone();
        effective.Validate();
    ...
    public ProgramTaskOptions GetOptions() => _options.Clone();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_options.Language != ProgramLanguage.Generic && genome.Language != _options.Language)
        {
            return new EvolutionTaskResult(EvolutionEvaluationStatus.Rejected,
                diagnostics: new[] { new EvolutionDiagnostic("program_language_mismatch", "Candidate language differs from the configured task language.") });
        }
        ...
        IProgramFitnessEvaluator evaluator)
    {
        ...
        {
            "program-evolution-task-v7-language-guard",

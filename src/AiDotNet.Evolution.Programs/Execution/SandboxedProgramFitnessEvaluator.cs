// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:src/Evolution/Programs/SandboxedProgramFitnessEvaluator.cs
...
#pragma warning restore CA1031
            ...
            if (cancellationToken.IsCancellationRequested) return Canceled(index + 1);

            if (response.Success && (response.ExitCode != 0 || response.ErrorCode is not null and not ProgramExecuteErrorCode.None ||
                response.Language != candidate.Language || response.CompilationAttempted || response.StdOut is null))
            {
                AddDiagnostic(diagnostics, index, "invalid_response", "The engine returned inconsistent execution evidence.");
                continue;
            }
        ...
        ProgramOutputComparison comparison)
    {
        ...
        {
            "program-sandbox-evaluator-v3-consistent-execution-evidence",

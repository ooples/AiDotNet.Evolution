// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:src/ProgramSynthesis/Execution/BoundedOutputReader.cs
...
internal sealed class BoundedOutputReader
{
    ...
    public async Task PumpAsync(StreamReader reader, CancellationToken cancellationToken)
    {
        ...
        {
            // A retained prefix is not proof of complete output.
        }
    ...
    public (string Text, bool Truncated) Snapshot()
    {
        lock (_gate)
        {
            return (_builder.ToString(), _truncated || !_complete);
        }
    }

// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:src/ProgramSynthesis/Execution/ProcessProgramExecutionEngine.cs
...
public sealed class ProcessProgramExecutionEngine : IProgramExecutionEngine, IProgramExecutionTelemetrySource, IDisposable
{
    ...
    /// <inheritdoc/>
    public string Id => "process-program-execution";
    /// <inheritdoc/>
    ...
    public ProcessProgramExecutionEngine(ProgramSandboxOptions options)
    {
        ...
        VersionHash = EvolutionHash.Combine(new[]
        {
            ...
            _workspaceRoot, _pinnedPath, _options.RuntimeVersion,
            // No provisioned runtime identity means no cross-instance cache/replay reuse.
            _options.RuntimeVersion == "local-runtime-unversioned" ? Guid.NewGuid().ToString("N") : "versioned",
            System.Text.Json.JsonSerializer.Serialize(_limits),
            System.Text.Json.JsonSerializer.Serialize(_options.Interpreters.OrderBy(pair => pair.Key)
                .Select(pair => new { pair.Key, pair.Value.Executable, pair.Value.RunArgumentTemplate, pair.Value.CompileArgumentTemplate }))
        });
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // Own all mutable request data before queuing; later edits cannot bypass validation.
            var snapshot = new ProgramExecuteRequest
            {
                Language = request.Language,
                SourceCode = request.SourceCode,
                StdIn = request.StdIn,
                CompileOnly = request.CompileOnly,
                AllowedLanguages = request.AllowedLanguages?.ToList() ?? new(),
                PreferredLanguage = request.PreferredLanguage,
                AllowUndetectedLanguageFallback = request.AllowUndetectedLanguageFallback
            };
            return await ExecuteCoreAsync(snapshot, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            lock (_lifecycle)
    ...
    private async Task<ProgramExecuteResponse> ExecuteCoreAsync(
        ProgramExecuteRequest request, CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (response.Success && !response.StdOutTruncated)
        {
            output = response.StdOut;
            return true;
        }

        errorMessage = response.Error ?? "Captured output was incomplete or truncated.";
        return false;
    }

    /// <summary>Refuses new work; admitted calls retain their permits until they finish or cancel.</summary>
    public void Dispose()
    {
        lock (_lifecycle)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            if (workspace is not null) TryDeleteDirectory(workspace);
        }
    }
        ...
        out ProgramLanguage language)
    {
        ...
        language = request.Language;
        if (request.AllowedLanguages is { Count: > 0 } &&
            request.AllowedLanguages.Any(value => value == ProgramLanguage.Generic || !Enum.IsDefined(value)))
        {
            return Rejected(language, ProgramExecuteErrorCode.InvalidRequest,
                "AllowedLanguages must contain only concrete, defined languages.");
        }

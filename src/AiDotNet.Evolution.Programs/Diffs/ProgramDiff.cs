// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:src/Evolution/Programs/ProgramDiff.cs
...
public static class ProgramDiff
{
    ...
    public static ProgramDiffParseResult Parse(string response, ProgramDiffOptions? options = null)
    {
        ...
        if (response.Length > effective.MaxResponseChars)
        {
            failures.Add(new ProgramDiffFailure(ProgramDiffFailureReason.LimitExceeded, "Response exceeds the configured parsing bound."));
    ...
    /// <see cref="ProgramTaskOptions.EnforceEvolveBlocks"/> is set, the marker pair that bounds legal edits;
    ...
    public static ProgramDiffApplyResult Apply(
        ...
        ProgramTaskOptions? options = null)
    {
        if (source is null) throw new ArgumentNullException(nameof(source));
        if (blocks is null) throw new ArgumentNullException(nameof(blocks));
        ProgramTaskOptions effective = options ?? new ProgramTaskOptions();
        effective.Validate();
        ...
        if (source.Length > effective.MaxProgramChars || blocks.Count > diffOptions.MaxBlocks)
        {
            failures.Add(new ProgramDiffFailure(ProgramDiffFailureReason.LimitExceeded, "Source or block count exceeds the configured bound."));
            return new ProgramDiffApplyResult(source, source, 0, failures, false);
        }

        for (int blockIndex = 0; blockIndex < blocks.Count; blockIndex++)
        {
            List<string> lines = ProgramText.SplitLines(modified);
            if (ProgramText.EndsWithNewLine(modified)) lines.RemoveAt(lines.Count - 1);
            ...
            if (string.IsNullOrWhiteSpace(block.SearchText) || searchLines.Count == 0)
            {
                failures.Add(new ProgramDiffFailure(
            ...
            if (effective.EnforceEvolveBlocks
                && !IsInsideEvolveBlock(modified, markers, match, match + searchLines.Count - 1))
            {
                failures.Add(new ProgramDiffFailure(
            ...
            // Slice the original text: joining all lines would rewrite protected mixed terminators.
            List<int> starts = ProgramText.LineStarts(modified);
            int start = starts[match];
            int lastLine = match + searchLines.Count - 1;
            int contentEnd = starts[lastLine] + lines[lastLine].Length;
            int end = lastLine + 1 < starts.Count ? starts[lastLine + 1] : modified.Length;
            string terminator = modified.Substring(contentEnd, end - contentEnd);
            string replacement = block.ReplaceText;
            if (replacement.Length > 0) replacement += terminator;
            if ((long)modified.Length - (end - start) + replacement.Length > effective.MaxProgramChars)
            {
                failures.Add(new ProgramDiffFailure(ProgramDiffFailureReason.LimitExceeded, "Edited source exceeds the configured bound.", block.Ordinal));
                continue;
            }
            string candidate = modified.Substring(0, start) + replacement + modified.Substring(end);
            if (effective.EnforceEvolveBlocks && !ProgramEditBoundary.PreservesProtectedText(source, candidate, markers))
            {
                failures.Add(new ProgramDiffFailure(ProgramDiffFailureReason.OutsideEvolveBlock, "The replacement damages markers or protected text.", block.Ordinal));
                continue;
            }
            modified = candidate;
            applied++;
    ...
    private static void AddBlock(
        ...
        int openLine)
    {
        string searchText = ProgramText.JoinLines(searchBuffer, ProgramText.LineFeedText, trailingNewLine: false);
        string replaceText = ProgramText.JoinLines(replaceBuffer, ProgramText.LineFeedText, trailingNewLine: false);
        ...
        int lastLine)
    {
        ...
        if (!extraction.IsWellFormed || !extraction.HasRegions) return false;

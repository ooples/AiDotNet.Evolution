// Relocated from ooples/AiDotNet d4535f7376888a7c2d35d7e6229494c4c60c0ac6:src/Evolution/Deployment/EvolutionDeploymentLifecycle.cs
...
public sealed class EvolutionDeploymentLifecycle
{
    ...
    private async Task<Prepared> PrepareAsync(EvolutionDeployableArtifact candidate, long epoch, CancellationToken token)
    {
        ...
        var evidence = new DeploymentValidationEvidence
        {
            ...
            Direction = _policy.Direction,
            MinimumMeanGain = _policy.MinimumMeanGain,
            MaximumPValue = _policy.MaximumPValue,
            MaximumP95LatencyRatio = _policy.MaximumP95LatencyRatio,
            Pairs = pairs
        };
        evidence.Validate();
        ...
        Prepared Rejected(string reason) => new(reason, candidate, incumbent, slot.Revision, epoch,
            ...
            {
                SchemaVersion = 1,
                Kind = "rejected-deployment-validation",
                CandidateId = candidate.Id,
                IncumbentId = incumbent.Id,
                EnvelopeKey = candidate.Envelope.Key,
                Reason = reason,
        ...
        IReadOnlyList<EvolutionDeploymentMeasurement> measurements, DateTimeOffset observedAt, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            string evidence = _registry.RetainEvidence(JsonSerializer.SerializeToUtf8Bytes(new
            {
                ...
                observed.Revision,
                EnvelopeKey = observed.Artifact.Envelope.Key,
                slot.ValidationEvidenceId,
                slot.CandidateEvidenceSide,
                _policy.MaximumQualityDrop,
                _policy.MaximumMonitoredLatencyRatio,
                _policy.ConsecutiveRegressions,

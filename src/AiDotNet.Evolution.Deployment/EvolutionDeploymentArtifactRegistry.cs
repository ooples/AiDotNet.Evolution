// Relocated from ooples/AiDotNet d4535f7376888a7c2d35d7e6229494c4c60c0ac6:src/Evolution/Deployment/EvolutionDeploymentArtifactRegistry.cs
...
public sealed class EvolutionDeploymentArtifactRegistry
{
    ...
    private void WriteSlot(string? active, string? prior, string envelopeKey, string evidenceId, string validationEvidenceId, bool candidateSide)
    {
        ...
        DeploymentEncoding.WriteNew(pending, JsonSerializer.SerializeToUtf8Bytes(new SlotDocument
        {
            ...
            ValidationEvidenceId = validationEvidenceId,
            CandidateEvidenceSide = candidateSide,
    ...
    private sealed class ArtifactDocument
    {
        ...
        internal static ArtifactDocument From(EvolutionDeployableArtifact artifact) => new()
        {
            ...
            RuntimeHash = artifact.Envelope.RuntimeHash,
            DeviceHash = artifact.Envelope.DeviceHash,
            CompilerHash = artifact.Envelope.CompilerHash,
            DatasetHash = artifact.Envelope.DatasetHash,
            WorkloadHash = artifact.Envelope.WorkloadHash,

...
def promote(initial, selected, diagnostics, selected_audits, original_audits, *, expected, search_failed=False, performance_confirmed=True):
    ...
    if type(performance_confirmed) is not bool:
        raise ValueError("Explicit performance confirmation decision required")
    fallback = bool(not performance_confirmed or search_failed or initial == selected or diagnostics["selected"]["status"] != "valid" or

...
def execute(plan_path, registered_sha256):
    ...
    try:
        sandbox = WarmDockerSandbox(plan["image"], root / "sandbox")
        for index, row in enumerate(report["rows"]):
            ...
            transport = CodexTransport(plan["codex"], plan["model"], cell / "model", cap)
            evaluator = WarmEvaluator(sandbox, task["metadata"]["class"], task["cases"], task["validate"],
                                      identity=digest(task["metadata"]), samples=plan["samples"])

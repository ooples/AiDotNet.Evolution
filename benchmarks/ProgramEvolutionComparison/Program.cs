...
namespace AiDotNet.Evolution.ProgramComparison;
...
internal static class Program
{
    private static async Task<int> Main(string[] args)
    {
        ...
        var variation = new LlmProgramVariationOperator(broker, programOptions, variationOptions, promptBuilder: promptBuilder);
        ...
        try
        {
            ...
            if (result.RetainedFailures.Count > 0 || observer.Evaluations.Any(evaluation => evaluation.Status == EvolutionEvaluationStatus.Failed))
                throw new InvalidDataException("Comparison work failed or could not be reconciled.");
            await JsonSerializer.SerializeAsync(output, new
            {
                ...
                limitations = "Native Evolution prompts with full rewrite/no retries; sequential one-island length archive. Broker receipts determine actual work. No candidate execution occurs in this host. Provider token usage remains in broker transport receipts."
            });
            return 0;
        }
...
internal sealed class BrokerClient : IProgramChatClient, IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        var payload = new
        {
            system = string.Join("\n", messages.Where(message => message.Role == ProgramChatRole.System).Select(message => message.Text)),
            messages = messages.Where(message => message.Role != ProgramChatRole.System)
        ...
        return new ProgramChatResponse(ProgramChatMessage.Assistant(result.GetString() ?? throw new InvalidDataException("Missing model text.")), modelId: ModelId);
    }

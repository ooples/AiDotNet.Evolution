...
- Set `EarlyStopping.Metric = EvolutionEarlyStoppingMetric.ParetoHypervolume` for front progress. Named evaluator metrics remain explicit optional stopping criteria. Hypervolume is exact for two and three objectives; higher dimensions throw rather than silently approximate. It uses **raw measured values**, fixed normalization and the reference point `(1,...,1)`. Occupancy/QD-score stopping is rejected for Pareto runs.
- Early-stopping readings are **three-valued**, and that is the contract rather than an implementation detail. Each committed batch reads the criterion once and the reading is `Improved`, `NotImproved` or `Unmeasurable`; only `NotImproved` charges patience. An empty feasible front is `Unmeasurable` (`EmptyFeasibleFront`), **not** volume zero, so a run cannot be stopped before its first feasible point exists. The same rule covers the other criteria: a named evaluator metric no evaluation reported (`MetricNotReported`), an empty archive for QD-score or best-quality (`EmptyArchive`), and archives with no cells for occupancy (`NoArchiveCells`). Every skipped reading is counted with its reason in `EvolutionRunResult.EarlyStopping`, and a run that took at least one reading and never once measured its criterion **throws** instead of returning a result, naming the criterion and the reasons.

...
| US-21 durable external work | Implementation acceptance verified; CI/review/dependency merge gates remain | Strict tickets, engine codec/context bridge, schema-2 work/ledger persistence, hardware matching, heartbeat/cancel/idempotent receipts, JSON/native IPC, TypeScript/Python and C ABI. 953/953/752 core tests, 139 native TypeScript tests, eight Python cases on each host and six C ABI cases pass; 91.31%/77.63% coverage; strict .NET 10 native engine/checkpoint probes and package-byte checks pass. [Pinned raw evidence and failed approaches](../benchmarks/evidence/external-work/8aaef1e/README.md). Delivery recovery retains unresolved liabilities; lost partial-batch search state requires an explicit fork, not automatic reconstruction or budget reset. No network authentication/exactly-once physical-execution claim. [Contracts](DURABLE_EXTERNAL_WORK.md). |
...
| US-23 warm starts | Core contracts verified by a gated campaign; cross-story gates remain | Bounded [seed repertoires](WARM_START_REPERTOIRES.md), twelve-facet revalidation, [sample provenance](MEASUREMENT_ORIGIN.md), [persistent evaluation storage](PERSISTENT_EVALUATION_REUSE.md), age/uncertainty/force-fresh decisions and store accounting implemented. A paired no-reuse ablation now drives the real engine and cache through cold/warm/force-fresh/expired phases with primary and replay executions, checked by `analyze_reuse.py` and a required `noisy-reuse` CI job; [retained 32-seed evidence](../benchmarks/evidence/noisy-reuse/3c0a680/README.md) ships with an offline verifier. Representative cross-system and physical-noise evidence, consumer integrations and published-package integration remain, tracked on US-24/US-04/US-06. |

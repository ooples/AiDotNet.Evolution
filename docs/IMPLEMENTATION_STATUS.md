...
| US-21 durable external work | Integrated; final pinned evidence/readiness checks in progress | Strict source tickets, engine codec/context bridge, schema-2 session-bound recovery, atomic work/ledger coordinator, hardware matching, heartbeat/cancel/idempotent receipts, bounded JSON protocol, native IPC and TypeScript/Python clients. 985/985/785 core tests, 139 native TypeScript tests and six Python tests pass; 92.95%/79.73% coverage. Recovery is delivery-only: a new engine cannot attach to old work, and lost search state requires an explicit fork retaining campaign liabilities. No automatic fork or network authentication. [Session contract](EXTERNAL_WORK_IDENTITY.md), [durable contract](DURABLE_EXTERNAL_WORK.md), [worker protocol](DURABLE_WORKER_PROTOCOL.md). |

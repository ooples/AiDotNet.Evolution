...
The subsequent strict-host slice passed **921 tests on net10.0** and **129 TypeScript tests with zero skips**, including real Windows x64 NativeAOT cross-session/retry cases and downgrade/missing-ticket fixtures. The local native build succeeded but emitted trimming/AOT warnings in existing core persistence/measurement/ownership paths; this is not a claim of warning-free AOT persistence or verified Linux/macOS execution.

This section listed the rest of US-21 as outstanding. It is no longer: persistent pending work, reservations and results; authoritative restart reconciliation; durable leases, heartbeats and cancellation; compatible-worker selection; and durable-protocol binding integration all landed at the pinned revision and are exercised by the suites and bindings recorded in [the durable external-work contract](DURABLE_EXTERNAL_WORK.md).

What genuinely remains is exact partial-batch continuation, which needs persisted proposal, operator and external-response provenance. Without that, a continuation must be labeled a fork rather than a resumption -- the library refuses to claim otherwise. A newest-valid algorithm-checkpoint fallback cannot authorize redispatch on its own either: it may omit work a newer, corrupted snapshot had already dispatched.

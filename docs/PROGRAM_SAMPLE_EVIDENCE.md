...
and runs on every standalone engine evaluation dispatch.
...
The evaluator meters `program_evidence_invocations` and `cache_store_invocations` as logical
...
and finite statistics. These contracts now compile against the shipping
`AiDotNet.Evolution.Programs` project in `AiDotNet.Evolution.CSharp.Tests` on net8.0
and net10.0, without an AiDotNet package or source reference. The archived
`docs/evidence/raw-program-samples/d9eb58f` receipts are unchanged historical
AiDotNet evidence, not test results for this migration. The standalone optional
package targets net8.0 and net10.0; it does not promise net471 compatibility.

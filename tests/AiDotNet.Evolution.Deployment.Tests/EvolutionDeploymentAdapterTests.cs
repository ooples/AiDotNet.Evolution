...
using AiDotNetConsumer::AiDotNet.Models.Options;
using AiDotNetConsumer::AiDotNet.Regression;
...
{
    [Fact]
    public async Task Invalid_model_pair_is_rejected_before_factory_or_evaluation()
    {
        int calls = 0;
        var evaluator = EvolutionDeploymentEvaluators.Model("tiny-v1", _ => { calls++; return new TinyModel(); },
            (TinyModel _, int _, CancellationToken _) => new ValueTask<EvolutionDeploymentMeasurement>(Measurement(1)));
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => evaluator(Program("base"), -1, CancellationToken.None).AsTask());
        Assert.Equal(0, calls);
    }

    [Fact]
    public async Task Private_program_search_rejects_language_changes_before_fitness()
    {
        int calls = 0;
        var retuner = EvolutionDeploymentRetuners.Program(_ => new ProgramDeploymentSearchOptions
        {
            ...
            CustomFitnessEvaluator = new DelegateProgramFitnessEvaluator(_ => { calls++; return 1; })
        ...
        Assert.Equal("base", artifact.ReadProgram().Source);
    ...
    public async Task AutoMLRetuning_PromotesReloadableTrainedWeightsUnderIndependentHoldoutValidation()
    {
        ...
        using var persistenceScope = AiDotNetConsumer::AiDotNet.Helpers.ModelPersistenceGuard.InternalOperation();
    ...
    private sealed class DeploymentVariation(ProgramLanguage language = ProgramLanguage.Generic) : IProgramVariationOperator
    {
        ...
        { cancellationToken.ThrowIfCancellationRequested(); Proposals++; return new(new ProgramGenome("winner", language)); }

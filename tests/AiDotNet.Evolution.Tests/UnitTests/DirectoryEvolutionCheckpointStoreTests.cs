...
public sealed class DirectoryEvolutionCheckpointStoreTests
{
    ...
    public async Task EnvelopeTamperingAndForeignRunsAreTreatedAsUnreadableRatherThanTrusted()
    {
        ...
        JsonObject document = Assert.IsType<JsonObject>(JsonNode.Parse(File.ReadAllText(second)));
        document["Quality"] = 99.0;
        File.WriteAllText(second, document.ToJsonString());

// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:tests/AiDotNet.Tests/UnitTests/Evolution/Programs/ProgramDiffTests.cs
...
public sealed class ProgramDiffTests
{
    ...
    {
        var options = new ProgramTaskOptions { Language = ProgramLanguage.CSharp };
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() => new ProgramTaskOptions { MaxProgramChars = 0 }.Validate());
    }

    [Fact]
    public void EvolutionOptionsRejectEitherHalfOfAnExplicitMarkerPair()
    {
        var startOnly = new ProgramTaskOptions { EvolveBlockStartMarker = "/* START */" };
        var endOnly = new ProgramTaskOptions { EvolveBlockEndMarker = "/* END */" };
        ...
        Assert.Equal(nameof(ProgramTaskOptions.EvolveBlockEndMarker), missingEnd.ParamName);
        Assert.Equal(nameof(ProgramTaskOptions.EvolveBlockStartMarker), missingStart.ParamName);
    }
}

// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:tests/AiDotNet.Tests/UnitTests/Evolution/Programs/ProgramEvolutionTaskTests.cs
...
public sealed class ProgramEvolutionTaskTests
{
    ...
    {
        var options = new ProgramTaskOptions { MaxProgramChars = 10 };
    ...
    public async System.Threading.Tasks.Task DamagedEvolveBlocksAreRejectedWhenEnforced()
    {
        ...
        EvolutionTaskResult missing = await task.EvaluateAsync(Candidate(new ProgramGenome("print(1)", ProgramLanguage.Python)), Context);
    ...
    public void VersionHashTracksOptionsAndDescriptors()
    {
        ...
        string differentLanguage = Task(options: new ProgramTaskOptions { Language = ProgramLanguage.CSharp }).VersionHash;
        string differentBound = Task(options: new ProgramTaskOptions { MaxProgramChars = 500 }).VersionHash;
        string enforced = Task(options: new ProgramTaskOptions { EnforceEvolveBlocks = true }).VersionHash;
    ...
    {
        var options = new ProgramTaskOptions { MaxProgramChars = 4096 };
        ProgramEvolutionTask task = Task(options: options);
...
#pragma warning restore CS8600, CS8625
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new ProgramEvolutionTask(evaluator, options: new ProgramTaskOptions { MaxProgramChars = -1 }));
    }
}

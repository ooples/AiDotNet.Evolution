// Migrated from ooples/AiDotNet 66d7602c92101e5ab2bd9db8cfa7f7526fa2c75d:tests/AiDotNet.Evolution.CSharp.Tests/CSharpProposalSourceTests.cs
...
{
    private static ResourceMeteredVariationOperator<Programs.ProgramGenome> Meter(CSharpProposalSource source, EvolutionResourceLedger ledger) =>
    ...
    public void Setup_denial_and_bad_reference_images_cannot_disappear_from_accounting()
    {
        ...
        Assert.Throws<EvolutionResourceBudgetException>(() => CSharpProposalSource.Create(client, options, Program(), denied));
        ...
        try
        {
            ...
            var ledger = Ledger();
            Assert.Throws<ArgumentException>(() => CSharpProposalSource.Create(client, options, Program(), ledger));
            ...
            Assert.Throws<BadImageFormatException>(() => CSharpProposalSource.Create(client, options, Program(), Ledger()));
    ...
    public async Task Cancellation_and_fatal_errors_after_model_dispatch_keep_unknown_maxima_and_propagate()
    {
        ...
        var client = new ScriptedClient();
        var ledger = Ledger();
        var source = CSharpProposalSource.Create(client, options, Program(), ledger);
        ...
        foreach (Exception failure in new Exception[] { new OperationCanceledException(), new OutOfMemoryException("synthetic") })
        {
            ...
            source = CSharpProposalSource.Create(client, Options(), Program(), ledger);
    ...
    {
        var source = CSharpProposalSource.Create(new ScriptedClient(), Options(), Program(), Ledger());
    ...
    public async Task Repair_history_and_reported_model_identity_remain_bounded()
    {
        ...
        var client = new ScriptedClient
        {
            Handler = (_, _) => new CompilerChatResponse(CompilerChatMessage.Assistant(new string('x', 600)),
            modelId: new string('m', 300), usage: new CompilerChatUsage(100, 50))
        };
        var ledger = Ledger();
        var source = CSharpProposalSource.Create(client, options, Program(), ledger);
    ...
    public async Task Unknown_provider_usage_is_not_free_and_raw_exception_text_is_not_exposed(bool missingUsage)
    {
        var client = new ScriptedClient
        {
            Handler = (_, _) => missingUsage ? new CompilerChatResponse(CompilerChatMessage.Assistant("{}")) : throw new IOException("SECRET_CREDENTIAL")
        };
        var ledger = Ledger();
        var source = CSharpProposalSource.Create(client, Options(), Program(), ledger);
        var metered = Meter(source, ledger);
    ...
    public async Task Actual_token_overrun_is_retained_and_blocks_further_ledger_admission()
    {
        ...
        var client = new ScriptedClient { Handler = (_, _) => ScriptedClient.Response("{}", new CompilerChatUsage(100, options.MaxOutputTokens + 1)) };
        var ledger = Ledger();
        var source = CSharpProposalSource.Create(client, options, Program(), ledger);
        await Assert.ThrowsAsync<InvalidOperationException>(() => Meter(source, ledger).ProposeAsync(Context()).AsTask());
        Assert.Single(client.Conversations);

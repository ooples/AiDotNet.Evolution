// Migrated from ooples/AiDotNet 9cd7d5d6c366a483874024650d02901f69a1829c:tests/AiDotNet.Tests/UnitTests/Evolution/Programs/Novelty/LlmProgramNoveltyJudgeTests.cs
...
public sealed class LlmProgramNoveltyJudgeTests
{
    ...
    public void VerdictParsingHandlesEverySpellingOfTheAnswer(string answer, ProgramNoveltyVerdict expected) =>
        Assert.Equal(expected, LlmProgramNoveltyJudge.ParseVerdict(answer));
    ...
    public void TheCanonicalNegativeAnswerIsNotReadAsItsOwnOpposite()
    {
        ...
        Assert.Equal(ProgramNoveltyVerdict.NotNovel, LlmProgramNoveltyJudge.ParseVerdict("NOT_NOVEL"));
        Assert.Equal(ProgramNoveltyVerdict.NotNovel,
            LlmProgramNoveltyJudge.ParseVerdict("NOT_NOVEL. The change is a rename only."));
    }
    ...
    public void TheEarliestVerdictTokenWinsWhenBothAppear()
    {
        Assert.Equal(ProgramNoveltyVerdict.Novel,
            LlmProgramNoveltyJudge.ParseVerdict("NOVEL. It is not novel only in the trivial sense."));
        Assert.Equal(ProgramNoveltyVerdict.NotNovel,
            LlmProgramNoveltyJudge.ParseVerdict("NOT_NOVEL, although a novel comment was added."));
    }
    ...
    public async Task JudgeReturnsTheParsedVerdictAndCountsItsRequests()
    {
        ...
        var judge = new LlmProgramNoveltyJudge(client);
    ...
    {
        var client = new FakeChatClient("NOVEL");
        var judge = new LlmProgramNoveltyJudge(client, maxProgramBytes: 512);
...
#pragma warning disable CS8625
        Assert.Throws<ArgumentNullException>(() => new LlmProgramNoveltyJudge(null));
#pragma warning restore CS8625
        Assert.Throws<ArgumentException>(() => new LlmProgramNoveltyJudge(client, id: "  "));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new LlmProgramNoveltyJudge(client, maxProgramBytes: 8));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new LlmProgramNoveltyJudge(client, maxOutputTokens: 0));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new LlmProgramNoveltyJudge(client, temperature: 3.0));
    }
}
